...
public interface LeaveRequestRepository extends JpaRepository<LeaveRequest, Long> {
    ...
    Page<LeaveRequest> findAllOrderByRequestDateDesc(Pageable pageable);

    @Query("SELECT lr FROM LeaveRequest lr " +
           "WHERE lr.employee.department.id = :departmentId " +
           "ORDER BY lr.requestDate DESC")
    Page<LeaveRequest> findByDepartment(@Param("departmentId") Long departmentId, Pageable pageable);

    @Query("SELECT lr FROM LeaveRequest lr " +
           "WHERE lr.employee.id = :employeeId " +
           "ORDER BY lr.requestDate DESC")
    Page<LeaveRequest> findByEmployeeIdOrderByRequestDateDesc(
            @Param("employeeId") Long employeeId, Pageable pageable);

    // ── Sin paginar — mantenidos para uso interno ─────────────────────────
